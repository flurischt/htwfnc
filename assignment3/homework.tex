...
$\pi = 3$ flops/cycle and $\beta = 28*1024^3/(3.5*1000^3) = \textbf{8.5}$ \textbf{bytes/cycle ???}
\subsection*{(a) / (b)}
insert plot here
\subsection*{(c)}
No, it's not possible. We can only execute two multiplications and one addition per cycle. For matrices with size $N \times N$ there are $3*N^3$ multiplications and $N^3$ additions. Assuming we can execute the two muls and one add for $N^3$ cycles there are still $N^3$ muls that need to be executed. Those can be executed in $\frac{N^3}{2}$ cycles leading to a new peak performance of $\pi_c= \frac{3N^3 + N^3}{N^3 + \frac{N^3}{2}}=\frac{4}{\frac{3}{2}}=\underline{\frac{8}{3}}$ flops/cycle.

\subsection*{(d)}
Aussuming N is sufficiently small such that the working set fits into cache the peak performance can be reached. There are 2 multiplications and one addition which is exactly what our cpu can execute per cycle.
\textbf{TODO: what about linear dependency? For the first elements we cannot execute 2muls and 1 add because we have to wait for the muls to finish...}
