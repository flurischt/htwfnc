 ...
 */
double get_cycles_time (comp_func f, smat_t *ax, int iters) {
 ...
 * Result is in MFLOPs
 */
double get_perf_score(comp_func f, unsigned int size, unsigned int iters) {
	smat_t *a;
	double mflops, cycles, seconds, fpc, ops;
	a = random_SMat(size);
	cycles  = get_cycles_time(f, a, iters);
 ...
 */
double perf_test(comp_func f, char *desc, unsigned int * test_sizes,
		unsigned int * test_iters, unsigned int num_tests)
{
	double results_sum, avg;
	static int first = 1;
	unsigned int i, test_size, test_iter;
	...
	for (i = 0; i < num_tests; i++) {
		...
		if (!check_valid(f, test_size) ) {
			invalid_msg(test_size, desc);
			return 0;
		}
		results_sum += get_perf_score(f, test_size, test_iter);
	}
	avg = results_sum / num_tests;
	printf("Average: %.4f F/C\n\n", avg);
	return avg;
}
